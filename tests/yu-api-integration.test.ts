...
test("YU API personnel lookup keeps credentials server-side and returns a minimal directory record", async () => {
  ...
  assert.deepEqual(await client.searchPersonnel("  alisher  "), [
    {
      id: "42",
  ...
  assert.match(requestedUrls[0], /search=alisher/);
  assert.equal(authorization, "Token service-token");
  assert.doesNotMatch(JSON.stringify(await client.searchPersonnel("alisher")), /identify_code|custom_data/);
