...
test("recipient search is normalized, active-only, self-excluding, minimal and bounded", () => {
  const users = [
    user(ACTOR_ID, "Алия Өмір", "self@example.com", "employee"),
    user("22222222-2222-4222-8222-222222222222", "  Әлия Жан  ", "aliya@example.com", "admin"),
  ...
  assert.equal(byName[0].fullName, "Әлия Жан");
  assert.deepEqual(
...
function user(id: string, fullName: string, email: string, role: UserDto["role"]): UserDto {
  return { id, code: `USR-${id.slice(0, 4)}`, fullName, email, phone: "+70000000000", role, emailVerified: true, active: true, version: 1, addedAt: "2026-08-01T00:00:00.000Z" };
}
